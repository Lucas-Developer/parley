...
class GPG(object):
    ...
    def search_keys(self, query, keyserver="pgp.mit.edu"):
        ...
        >>> assert result
        >>> result = gpg.search_keys('<vinay_sajip@hotmail.com>','keyserver.ubuntu.com')
        >>> assert result

        """

        args = "--fixed-list-mode --fingerprint --with-colons --keyserver %s --search-keys \"%s\"" % (keyserver, query)
        args = [args]
        p = self._open_subprocess(args)
    ...
    but this was quicker, easier, and (IMHO) more readable.
    '''
